...
The lane-separation invariant — that data lanes (`gsxdb-lane`) cannot directly mutate state lanes without going through the bridge (`gsxdb-bridge`) — is **load-bearing**. Any PR that touches the bridge crates, the lane-separation script, or `deny.toml` linting rules deserves explicit review.
...
git diff main...HEAD -- gsxdb-lane/ gsxdb-bridge/ scripts/check-lane-separation.sh deny.toml
```
...
1. Enumerate all modules in `gsxdb-lane/`
2. For each, statically check that no item imports from `gsxdb-state` directly (only `gsxdb-bridge`)
...
```
File                         Risk      Notes
gsxdb-bridge/src/lib.rs      Low       Added new approve_anchor() — bridge-mediated, OK
gsxdb-lane/src/queue.rs      Medium    Imports from gsxdb-state — verify it goes through bridge
