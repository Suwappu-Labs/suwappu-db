...
You have **34 repositories** in `suwappu`. **Two
...
| EVM execution | exists in `gsx-revm` | Wiring gsx-revm ↔ suwappudb-bridge |
...
### Option B — Wire MonadBFT consensus + gsx-revm + suwappu-db into a minimal L1 devnet (weeks–months)
...
2. Wire `gsx-revm` into `suwappudb-bridge::vm::executor` (replace
...
- Listed all repos under `suwappu` via `gh repo list`
