...
|---|---|---|---|---|---|
| A-001 | `crates/gsxdb-lane/src/lib.rs:32` | "Phase-1 placeholder" mempool wording | intentional-defer | Replace FIFO mempool with S5+/prod queue semantics | S9 |
| A-002 | `crates/gsxdb-bridge/src/recovery/replay.rs:79-80` | incremental parent source described as placeholder | intentional-defer | Define/implement checkpoint start contract for non-zero replay | S12 (IQ-9) |
| A-003 | `crates/gsxdb-state/src/tree/ops.rs:127` | commitment placeholder branch | resolved | Removed placeholder state; verification now uses `Option<Commitment>` path directly | S8.5 (done) |
| A-004 | `crates/gsxdb-state/src/tree/commit.rs:37` | const placeholder commitment | intentional-defer | Replace with real commitment backend under Verkle swap | S10 |
...
| A-009 | `crates/gsxdb-bridge/src/vm/mod.rs:9` | mock executor caveat | intentional-defer | Add real Move VM adapter and migration harness | S9 |
| A-010 | `crates/gsxdb-bridge/src/anchor/mod.rs:14` | on-chain registry/signatures deferred | intentional-defer | Implement Solidity registry + signature parity matrix | S11 |
