//! **gsxdb-types** — frozen public-type surface for downstream
//! consumers of gsx-db.
//!
//! This crate is a **re-export facade**. The actual type definitions
//! live in `gsxdb-state` and `gsxdb-bridge`; this crate stabilises the
//! subset that the [`INTEGRATORS.md`](https://github.com/GlobalSettlementNetwork/gsx-db/blob/main/INTEGRATORS.md)
...
//! ## Versioning
//!
//! `gsxdb-types` carries its own `version` in Cargo.toml, independent
...
//! ```toml
//! [dependencies]
//! gsxdb-types = { git = "https://github.com/GlobalSettlementNetwork/gsx-db", tag = "v0.1.0-pre" }
//! ```
//!
//! Pin to a tag for reproducible builds. Workspace internals do not
//! depend on `gsxdb-types`; only downstream code does.
...
/// State-tree primitives — see [`gsxdb_state::tree`] for the full
/// trait surface. Re-exported for inclusion-proof verifiers.
pub mod tree {
    pub use gsxdb_state::tree::{
        commit::CommitmentScheme, Blake3Scheme, Commitment, Node, Proof, ProofStep, StateTree,
    };
}

/// State snapshot capture / restore — see [`gsxdb_state::snapshot`].
/// `SNAPSHOT_ENTRY_BYTES` is added in S12.2; pre-merge consumers get
/// `StateSnapshot` + `SnapshotManager` only.
pub mod snapshot {
    pub use gsxdb_state::snapshot::{SnapshotManager, StateSnapshot};
}

/// DAG store + block-shape primitives — see [`gsxdb_state::dag`].
pub mod dag {
    pub use gsxdb_state::dag::{BlockHash, DagBlock, DagStore};
}

/// Metrics surface — see [`gsxdb_state::Metrics`].
pub mod metrics {
    pub use gsxdb_state::{Counter, Gauge, Histogram, Metrics, Timer};
}
...
// since the re-export sites are already prepared in `gsxdb-bridge`'s
// module root.
pub use gsxdb_bridge::anchor::{
...
pub use gsxdb_bridge::anchor::types::AuthScheme;
