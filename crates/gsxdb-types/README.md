...
**Frozen public-type surface for gsx-db.** This is the crate
downstream wallets, indexers, parallel verifiers, and chain-listeners
depend on.

`gsxdb-types` is a thin re-export facade over `gsxdb-state` and
`gsxdb-bridge`. It carries its own [`version`](./Cargo.toml) so the
...
- You're modifying gsx-db itself.
