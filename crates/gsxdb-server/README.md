...
- `/v1/rpc` is **optionally** gated behind
  `Authorization: Bearer <token>` when `GSXDB_BEARER_TOKEN` is set
...
docker run --rm \
    ...
    -e GSXDB_BEARER_TOKEN=$(openssl rand -hex 32) \
    -e RUST_LOG=info \
    ghcr.io/globalsettlementnetwork/gsx-db:v0.1.0-pre
```
...
|---|---|---|
| `GSXDB_BEARER_TOKEN` | (unset) | Enables bearer-token auth on `/v1/rpc` |
| `RUST_LOG` | (unset) | Standard tracing-subscriber filter |
| `CONFIG_PATH` | `/etc/gsxdb/config.toml` | Reserved for future TOML config file |
