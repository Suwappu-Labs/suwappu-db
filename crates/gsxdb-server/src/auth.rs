...
//!
//! Default off: with no `GSXDB_BEARER_TOKEN` env var set, the
...
//! `GSXDB_BEARER_TOKEN` get a hard reject for any request without a
...
impl BearerAuthConfig {
    /// Read `GSXDB_BEARER_TOKEN` from the environment. If unset (or
    /// empty), returns a `disabled` config that lets every request
    /// through.
    #[must_use]
    pub fn from_env() -> Self {
        match env::var("GSXDB_BEARER_TOKEN") {
