...
//!
//! Suwappu-DB delegates Move bytecode execution to an abstract executor trait.
...
//! ## Invocation site
//!
//! The Move executor is invoked from `suwappudb-bridge::bundle::BundleExecutor`
...
/// Address of the canonical suwappu-db `Coin<T>` module (`0x1::coin`).
...
/// - **S9.5c** — compile + bundle the canonical suwappu-db `Coin<T>` Move
...
/// branch (per the suwappu-dag lane-auditor convention — `version = "*"` +
...
impl MoveExecutor for AptosMoveExecutor {
    fn execute(
        ...
        use crate::vm::aptos_session::{decode_coin_store, BalanceViewResolver, SuwappudbModuleBytes};
        ...
        let bytes_storage = SuwappudbModuleBytes {
            store: modules,
        ...
        // 5. Load the entry function. Convert suwappu-db's (MoveAddress, Identifier)
        ...
        let loaded = loader
            .load_instantiated_function(
                ...
                &aptos_module_id,
                &aptos_function_name,
                &[], // type args — S9.5g converts suwappu TypeTag list
            )
