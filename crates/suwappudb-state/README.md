...
> not on this crate directly. `suwappudb-state` is internal — its public
...
`suwappudb-state` is the *only* crate that owns canonical state, and
the *only* writer is `suwappudb-bridge` (guarded by the `BridgeToken`
capability). `suwappudb-lane` (ingest) cannot import `suwappudb-state`. This
...
```sh
cargo test -p suwappudb-state                       # 128 unit/integration
cargo test -p suwappudb-state --features production-verkle      # +Verkle paths
PROPTEST_CASES=10000 cargo test -p suwappudb-state --release   # exit-gate
```
