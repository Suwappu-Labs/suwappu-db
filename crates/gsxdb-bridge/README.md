...
**The only writer to `gsxdb-state`.** Validates intents, runs OCC
...
> not on this crate directly. `gsxdb-bridge` is internal — its public
> APIs may evolve in any pre-1.0 minor bump.

## What it owns

- **Intent validation** (`Bridge::submit`) — the type-system gate
  that mints a `BridgeToken` and applies it to `gsxdb-state`.
...
| `production-move-executor` | Propagates the same flag to `gsxdb-state` | off |

## Tests

```sh
cargo test -p gsxdb-bridge                                      # 164 unit/integration
...
PROPTEST_CASES=10000 cargo test -p gsxdb-bridge --release       # exit-gate
```
