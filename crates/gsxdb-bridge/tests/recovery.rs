...
proptest! {
    ...
    fn recover_matches_live_state(
        ...
        gsxdb_bridge::replay(&store, &mut replayed, &ContractRegistry::new(), 0).unwrap();
    ...
    fn replay_is_deterministic(
        ...
        gsxdb_bridge::replay(&store, &mut a, &ContractRegistry::new(), 0).unwrap();
        gsxdb_bridge::replay(&store, &mut b, &ContractRegistry::new(), 0).unwrap();
    ...
    fn tampered_state_root_caught(
        ...
        let mut replayed = seeded_state();
        let result = gsxdb_bridge::replay(
            &tampered_store,
            &mut replayed,
