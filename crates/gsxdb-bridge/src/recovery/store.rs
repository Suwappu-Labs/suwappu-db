...
mod tests {
    ...
    fn redb_corrupt_payload_is_rejected_without_panic() {
        ...
        {
            let mut blocks = write_txn.open_table(BLOCKS_BY_HASH).expect("open blocks table");
            // truncated payload (just version byte + a few bytes)
            blocks
                .insert(bad_hash, [BLOCK_ENCODING_VERSION, 1, 2, 3].as_slice())
                .expect("insert bad payload");
            let mut heights = write_txn.open_table(HEIGHT_TO_HASH).expect("open heights table");
            heights.insert(7, bad_hash).expect("insert bad height->hash");
        }
    ...
    fn redb_aborted_write_txn_leaves_no_partial_state() {
        ...
        // Simulate a crash/failure after writing one table but before commit:
        // write to blocks_by_hash only, then drop txn without commit.
