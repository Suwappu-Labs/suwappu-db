...
use suwappudb_bridge::{Bundle, BundleExecutor, BundleStep, RejectReason, TxOutcome};
use suwappudb_state::{
...
proptest! {
        ...
        for step in &bundle.steps {
            let intent = match step {
                BundleStep::Evm(tx) => {
                    ...
                    suwappudb_bridge::Intent::Transfer { from: c.from, to: c.to, amount: c.amount }
                }
                BundleStep::Move(tx) => {
                    let c = tx.to_canonical();
                    suwappudb_bridge::Intent::Transfer { from: c.from, to: c.to, amount: c.amount }
            ...
            let mut bridge = suwappudb_bridge::Bridge::new(&mut probe);
    ...
    fn first_step_failure_is_pure_noop(
        ...
        prop_assert_eq!(
            result.outcome,
            suwappudb_bridge::BundleOutcome::Reverted { failed_step: 0 }
        );
        prop_assert_eq!(
