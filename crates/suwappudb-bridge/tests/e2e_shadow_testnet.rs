...
//!
//! Demonstrates the full Suwappu-DB stack:
...
fn anchor_parity_detects_divergence() {
    ...
    match parity_result {
        ...
        suwappudb_bridge::ParityResult::Disagreed { .. } => {
...
fn shadow_testnet_bootstrap() {
    ...
    assert!(
        prometheus_text.contains("suwappudb_block_height"),
        "prometheus export should contain block_height"
    );
    assert!(
        prometheus_text.contains("suwappudb_blocks_committed"),
