...
use crate::TxOutcome;
use suwappudb_state::{EvmTx, Identifier, MoveAddress, MoveCall, MoveTx};

/// One step within a bundle.
///
/// `Evm` + `Move` flavours decode to the same canonical `Intent` via
/// the existing `to_canonical` helpers in `suwappudb-state::vm::tx`. The
...
pub enum BundleStep {
    ...
    /// [`suwappudb_state::MoveExecutor`]; resource writes apply to substrate
    /// at bundle commit.
    MoveCall(MoveCall),
