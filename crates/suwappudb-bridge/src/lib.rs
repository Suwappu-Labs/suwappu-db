//! The bridge between `suwappudb-lane` (untrusted data ingest) and `suwappudb-state`
//! (authoritative state).
//!
...
//! [`suwappudb_state::StateChange`] that is applied through a `BridgeToken` only
//! this crate can mint.
//!
...
// suwappudb-bridge crate is the only capability-gated mutation surface;
...
pub enum Intent {
    ...
    DeployModule {
        ...
        account: suwappudb_state::MoveAddress,
        /// Module name. Validated as a Move [`Identifier`] before
        /// reaching the lane.
        name: suwappudb_state::Identifier,
...
impl<'s> Bridge<'s> {
    ...
    /// anchor, governance registry) — the surface suwappu-dag's
    /// `SuwappuDbSubstrate` needs to un-stub its L2 / DA / governance intent
...
mod tests {
    use super::*;
    use suwappudb_state::BridgeToken;
    ...
    fn deploy_module_routes_to_bundle_executor() {
        ...
        let result = bridge.submit(Intent::DeployModule {
            account: suwappudb_state::MoveAddress(account_bytes),
            name: suwappudb_state::Identifier::new("hello_module").unwrap(),
