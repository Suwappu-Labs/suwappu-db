...
//! eventually hands them off to `suwappudb-bridge` for validation and state
//! mutation.
//!
//! # Lane-separation invariant
//!
//! This crate must never `use suwappudb_state::*` and must never list
//! `suwappudb-state` as a Cargo dependency. The only path from a lane to state
//! is through a [`suwappudb_bridge::Bridge`] handle passed in by the caller.
//!
//! Enforcement:
//! - `Cargo.toml` of this crate has no `suwappudb-state` dep
//! - `scripts/check-lane-separation.sh` rejects any `use suwappudb_state` in
//!   this crate's source

#![deny(missing_docs)]

use suwappudb_bridge::{Bridge, Intent, RejectReason};
...
mod tests {
    use super::*;
    // NOTE: this test imports from suwappudb_bridge to construct test fixtures.
    // It must NOT import from suwappudb_state — and it doesn't, because the
    ...
    fn drain_empty_stays_empty() {
        let mempool = Mempool::new();
        // We can't construct a State here without depending on suwappudb-state,
