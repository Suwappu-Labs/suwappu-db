//! suwappudb-server — HTTP server for state queries and RPC endpoints.
//!
...
async fn metrics() -> (StatusCode, String) {
    (
        StatusCode::OK,
        "# HELP suwappudb_info Suwappu-DB server info\n".to_string(),
    )
}
...
async fn main() -> anyhow::Result<()> {
    ...
    info!("Starting suwappudb-server on port {}", config.port);
